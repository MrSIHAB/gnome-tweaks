...
# Light Image is 'EdgeGlassLight.png'
# Dark Image is 'EdgeGlassDark.png'
# XML File is 'EdgeGlass.xml'
# All the files are situated in './{Files and Image name}'

LIGHT_IMAGE="EdgeGlassLight.png"
DARK_IMAGE="EdgeGlassDark.png"
XML_FILE="EdgeGlass.xml"
...
if [[ "$EUID" -eq 0 ]]; then
    ...
    IMG_DEST="/usr/share/backgrounds/EdgeGlass"
else
    XML_DEST="$HOME/.local/share/gnome-background-properties"
    IMG_DEST="$HOME/.local/share/backgrounds/EdgeGlass"
fi
...
if [[ "$1" == "-r" || "$1" == "--remove" ]]; then
    ...
    echo "Edge Glass wallpaper removed successfully."
    exit 0
fi
...
echo "Edge Glass wallpaper installed successfully."
